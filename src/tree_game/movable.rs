...
pub trait Movable {
    ...
    fn change_position(&mut self, tree: &Tree, change_vec: Vector2<f64>) {
        ...
        let current_branch = tree.get_branches()
            .get(&current_position.get_branch_index())
            .expect("Movable Object Not on a Valid Branch!");

        let new_logical_pos = current_position.get_branch_position() + change_vec;
        ...
        if new_logical_pos.x < 0.0 {
            if let Some(connection) = current_branch.get_connection(new_logical_pos.y, super::tree::BranchSide::Left) {
                ...
                let new_branch_id = connection.get_id();
                ...
                let new_position = Position::new(new_branch_id, Vector2::new(new_branch.get_logical().get_total_length(), new_branch_boundary_pos));

                self.set_position(new_position);
                // debug(&format!("Player Jumped to the left: Old Position: Branch:{:?} x:{:?} y:{:?}, \n New Position: Branch:{:?} x:{:?} y:{:?}, \n Jump: x:{:?} y:{:?}",
                //                current_position.get_branch_index(), current_position.get_branch_position().x, current_position.get_branch_position().y,
                //                new_position.get_branch_index(), new_position.get_branch_position().x, new_position.get_branch_position().y,
                //                change_vec.x, change_vec.y));
            }
            else {
                let new_position = Position::new(current_position.get_branch_index(), Vector2::new(0.0, new_logical_pos.y));
                self.set_position(new_position);
            }
        }
        else if new_logical_pos.x > current_branch.get_logical().get_total_length() {
            if let Some(connection) = current_branch.get_connection(new_logical_pos.y, super::tree::BranchSide::Right) {
                ...
                let new_position = Position::new(current_position.get_branch_index(), Vector2::new(current_branch.get_logical().get_total_length(), new_logical_pos.y));
                self.set_position(new_position);
            ...
            let new_position = Position::new(current_position.get_branch_index(), new_logical_pos);
            self.set_position(new_position);
        }
    }
}
